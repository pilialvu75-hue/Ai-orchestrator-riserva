...
void main() {
  test('resource profile reaches extended native creation ABI', () async {
    final dir = await Directory.systemTemp.createTemp('session-config-test-');
    try {
      final path = '${dir.path}/config.so';
      final build = await Process.run('cc', [
        '-shared',
        '-fPIC',
        'test/fixtures/session_resource_config.c',
        '-o',
        path,
      ]);
      expect(build.exitCode, 0, reason: '${build.stderr}');
      final signature = await createNativeSessionOffUi(
        'model.gguf',
        nGpuLayers: 0,
        nCtx: 2048,
        nThreads: 2,
      ...
      expect(signature, 2048 * 1000000 + 128 * 1000 + 32);
    } finally {
  ...
  test(
    'native release waits off the caller isolate and is awaited to completion',
    () async {
