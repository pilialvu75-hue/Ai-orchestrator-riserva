...
void main() {
  group('WorkshopAirLabWorkspacePromoter', () {
    ...
    setUp(() async {
      temp = await Directory.systemTemp.createTemp('airlab-promotion-');
    });
    ...
    test(
      'promotes staged create/update/delete into VirtualWorkspace but not the real gateway',
      () async {
        ...
        final stagingRoot = Directory('${temp.path}/staging');
        await Directory('${stagingRoot.path}/lib').create(recursive: true);
        await File('${stagingRoot.path}/lib/app.dart')
            .writeAsString('new-app');
        await File('${stagingRoot.path}/lib/new.dart')
            .writeAsString('new-file');
        ...
        expect(session.workspace.read('lib/new.dart'), 'new-file');
        expect(session.workspace.contains('lib/remove.dart'), isFalse);

        // Promotion is staging -> VirtualWorkspace only.
        expect(gateway.files['lib/app.dart'], 'old-app');
        expect(gateway.files.containsKey('lib/new.dart'), isFalse);
        expect(gateway.files['lib/remove.dart'], 'remove-me');
        ...
        // The existing authoritative approval/apply path remains required.
    ...
    test('rejects provenance mismatch before VirtualWorkspace mutation', () async {
      final gateway = _RecordingGateway(
      ...
      await expectLater(
        ...
        throwsA(
          isA<WorkshopAirLabPromotionException>().having(
            (error) => error.code,
            'code',
      ...
      expect(session.workspace.read('lib/app.dart'), 'old-app');
      expect(gateway.writeCalls, 0);
      expect(gateway.deleteCalls, 0);
    });

    test('rejects oversized staged tampering before VirtualWorkspace mutation',
        () async {
      final gateway = _RecordingGateway(
        files: <String, String>{'lib/app.dart': 'old-app'},
      );
      ...
      await Directory('${stagingRoot.path}/lib').create(recursive: true);
      await File('${stagingRoot.path}/lib/app.dart')
          .writeAsString('tampered-content');
    ...
    test('rejects traversal changed paths before staging inspection', () async {
      final gateway = _RecordingGateway(
        files: <String, String>{'lib/app.dart': 'old-app'},
      );
      final session = await _session(gateway);
      ...
      final result = _airLabResult(
        changedFiles: const <String>['../outside.dart'],
      );
