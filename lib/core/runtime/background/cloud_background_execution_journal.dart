...
final class CloudBackgroundExecutionJournal {
  ...
  Future<void> settle(CloudBackgroundExecutionRecord? record) {
    ...
    return _serialize(() async {
      try {
        ...
        // Journal cleanup is observability/recovery support. A persistence
        // failure must never alter the already-completed inference result.
        debugPrint(
  ...
  /// Returns and consumes records for [sessionId] that were created by a
  /// previous process boot. Current-boot in-flight jobs are never consumed.
  Future<List<CloudBackgroundExecutionRecord>>
    ...
    return _serialize(() async {
      try {
        ...
        if (interrupted.isEmpty) return const <CloudBackgroundExecutionRecord>[];
