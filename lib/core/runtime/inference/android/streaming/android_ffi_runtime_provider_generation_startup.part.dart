...
extension AndroidFfiRuntimeGenerationStartupExtension on AndroidFfiRuntimeProvider {
  Future<_GenerationStartupState?> _prepareGenerationStartup({
    ...
    if (modelPath != null &&
      ...
      AndroidFfiRuntimeProvider._log(
        '[MODEL_PATH_RESOLVED] original=${_normalizePathForLogs(modelPath)} resolved=${_normalizePathForLogs(resolvedModelPath)}',
      );
    }
    if (resolvedModelPath == null ||
      ...
      AndroidFfiRuntimeProvider._log(
        '[FFI_BRANCH] session=$sessionId name=request_validation_missing_path_or_id',
      );
      AndroidFfiRuntimeProvider._log(
        '[MODEL_PATH] ABORT: path or modelId is null/empty',
      );
      AndroidFfiRuntimeProvider._log(
        '[TERMINAL_STATE] state=modelMissing reason=missing_path_or_id',
      );
    ...
    AndroidFfiRuntimeProvider._log(
      '[MODEL_EXISTS] path=$resolvedModelPath exists=$modelExists',
    );
    if (modelExists) {
      ...
      AndroidFfiRuntimeProvider._log(
        '[MODEL_SIZE] path=$resolvedModelPath size_bytes=$modelSizeBytes',
      );
      AndroidFfiRuntimeProvider._log(
        '[MODEL_READABLE] path=$resolvedModelPath readable=$modelReadable',
      );
    } else {
      AndroidFfiRuntimeProvider._log(
        '[MODEL_SIZE] path=$resolvedModelPath size_bytes=N/A (file not found)',
      );
      AndroidFfiRuntimeProvider._log(
        '[MODEL_READABLE] path=$resolvedModelPath readable=false (file not found)',
      );
    }
    if (!_androidSafeModelIds.contains(modelId) &&
        !_isImportedModelSafeForAndroid(modelId)) {
      if (_isDeveloperMode) {
        AndroidFfiRuntimeProvider._log(
        ...
        AndroidFfiRuntimeProvider._log(
          '[FFI_RUNTIME_UNAVAILABLE_REASON] session=$sessionId reason=developer_mode_unvalidated_model modelId=$modelId',
        );
      } else {
        _classifyFirstTokenTermination(
          flowState: flowState,
        ...
        AndroidFfiRuntimeProvider._log(
          '[FFI_BRANCH] session=$sessionId name=unsupported_model_guard',
        );
        ...
        AndroidFfiRuntimeProvider._log(
          '[TERMINAL_STATE] state=failed reason=unsupported_model modelId=$modelId',
        );
        _updateRuntimeStatus(
    ...
    AndroidFfiRuntimeProvider._log(
      '[MODEL_VALIDATION_BEGIN] session=$sessionId task=model_validation',
    );
    String? modelValidationError;
    try {
      modelValidationError =
          AndroidFfiRuntimeProvider._validateModelFileForRuntime(
        resolvedModelPath,
      );
      AndroidFfiRuntimeProvider._log(
        '[MODEL_VALIDATION_OK] session=$sessionId task=model_validation',
      );
      ...
      AndroidFfiRuntimeProvider._log(
        '[MODEL_VALIDATION_FAIL] session=$sessionId task=model_validation error=$error',
      );
      AndroidFfiRuntimeProvider._log(
        '[FFI_EXCEPTION] session=$sessionId stage=model_validation stack=$stackTrace',
      );
      await _fatalEarlyExit(
        flowState: flowState,
        controller: controller,
        ...
        reason:
            'Model validation threw unexpectedly before first FFI call: $error',
    ...
    if (modelValidationError != null) {
      ...
      AndroidFfiRuntimeProvider._log(
        '[FFI_BRANCH] session=$sessionId name=model_validation_failed',
      );
      AndroidFfiRuntimeProvider._log(
        '[GGUF] validation=failed path=$resolvedModelPath reason=$modelValidationError',
      );
      AndroidFfiRuntimeProvider._log(
        '[TERMINAL_STATE] state=failed reason=model_validation path=$resolvedModelPath error=$modelValidationError',
      );
    ...
    AndroidFfiRuntimeProvider._log(
      '[GGUF] validation=ok path=$resolvedModelPath',
    );
    if (!isForensicSelfTest) {
      ...
      if (!warmupReady) {
        AndroidFfiRuntimeProvider._log(
          '[FFI_BRANCH] session=$sessionId name=warmup_failed_non_blocking_continue',
        );
      }
    } else {
      AndroidFfiRuntimeProvider._log(
        '[WARMUP] skip session=$sessionId reason=self-test owns first token contract',
      );
    }
    if (!_ensureLibraryLoaded()) {
      ...
      AndroidFfiRuntimeProvider._log(
        '[FFI_BRANCH] session=$sessionId name=library_load_failed',
      );
      AndroidFfiRuntimeProvider._log(
        '[TERMINAL_STATE] state=ffiMissing reason=library_load_failed',
      );
      clearRuntimeVerification();
      _updateRuntimeStatus(
        LocalRuntimeStatus.ffiMissing,
        message: 'libllama_bridge.so is missing for this Android build.',
      );
      await _fatalEarlyExit(
        flowState: flowState,
        controller: controller,
    ...
    AndroidFfiRuntimeProvider._log(
      '[NATIVE_MODEL_LOAD_BEGIN] path=$resolvedModelPath modelId=$modelId'
    ...
    try {
      ...
      AndroidFfiRuntimeProvider._log(
        '[FIRST_FFI_CALL_BEGIN] stage=session_create phase=$_currentFfiPhase',
      );
      AndroidFfiRuntimeProvider._log(
        '[FFI_PRE_CREATE_SESSION] session=$sessionId path=$resolvedModelPath',
      );
      AndroidFfiRuntimeProvider._log(
        '[FORENSIC_BEFORE_CREATE_SESSION] sessionId=$sessionId modelId=$modelId modelPath=$resolvedModelPath',
      );
      AndroidFfiRuntimeProvider._log(
      ...
      AndroidFfiRuntimeProvider._log(
        '[FFI_CREATE_SESSION] path=$resolvedModelPath execution=worker_isolate',
      );
      ...
      AndroidFfiRuntimeProvider._log(
        '[FFI_POST_CREATE_SESSION] session=$sessionId native_session=$nativeSessionId',
      );
    } catch (error) {
      _classifyFirstTokenTermination(
        flowState: flowState,
        reason: 'session_create_exception',
        boundary: 'session_create',
        exception: true,
      );
      AndroidFfiRuntimeProvider._log(
        '[FFI_EXCEPTION] session=$sessionId stage=session_create error=$error',
      );
      AndroidFfiRuntimeProvider._log(
        '[SESSION_CREATE_FAIL] path=$resolvedModelPath exception=$error',
      );
      AndroidFfiRuntimeProvider._log(
        '[TERMINAL_STATE] state=failed reason=session_create_exception',
      );
      _setPhase(RuntimePhase.failed);
      _updateRuntimeStatus(
        LocalRuntimeStatus.failed,
        message: 'Session create failed: $error',
      );
    ...
    AndroidFfiRuntimeProvider._log(
      '[SESSION_CREATE_OK] session=$nativeSessionId path=$resolvedModelPath',
    );
    AndroidFfiRuntimeProvider._log(
      '[FFI_CREATE_SESSION_OK] session=$nativeSessionId path=$resolvedModelPath',
    );
    final activeAfterCreate = bindings.sessionIsActive(nativeSessionId);
    AndroidFfiRuntimeProvider._log(
      '[NATIVE_MODEL_LOAD_RESULT] llb_session_is_active after create: $activeAfterCreate',
    );
    if (nativeSessionId <= 0 || activeAfterCreate != 1) {
      ...
      AndroidFfiRuntimeProvider._log(
        '[FFI_BRANCH] session=$sessionId name=session_create_invalid_or_inactive',
      );
      final errMsg =
          AndroidFfiRuntimeProvider._safeLastError(bindings, nativeSessionId);
      AndroidFfiRuntimeProvider._log(
        '[SESSION_CREATE_FAIL] code=$nativeSessionId error=$errMsg path=$resolvedModelPath',
      );
      AndroidFfiRuntimeProvider._log(
        '[TERMINAL_STATE] state=failed reason=session_create_error code=$nativeSessionId',
      );
    ...
    AndroidFfiRuntimeProvider._log(
      '[NATIVE_MODEL_LOAD_SUCCESS] path=$resolvedModelPath modelId=$modelId'
      ' session=$nativeSessionId',
    );
    AndroidFfiRuntimeProvider._log(
      '[NATIVE_CONTEXT_CREATE] path=$resolvedModelPath status=ok',
    );
    ...
    final boundedMaxTokens = requestedMaxTokens.clamp(
      1,
      AndroidFfiRuntimeProvider._safeMaxTokens,
    );

    var effectiveRequest = request;
    NativeTokenContextBudgetResult? contextBudget;

    if (!isForensicSelfTest) {
      try {
        contextBudget = NativeTokenContextBudget.select(
          context: request.context,
          renderPrompt: (candidateContext) {
            final candidateComposed = _composePrompt(
              request.copyWith(context: candidateContext),
              modelId: modelId,
            );
            final candidateMetadata =
                SamplingMetadata.fromPrompt(candidateComposed);
            return candidateMetadata.stripFrom(candidateComposed);
          },
          countTokens: (candidatePrompt) =>
              bindings.countTokens(nativeSessionId, candidatePrompt),
          nCtx: LlamaNativeDefaults.nCtx,
          requestedGenerationTokens: boundedMaxTokens,
          safetyMargin: LlamaNativeDefaults.promptTokenSafetyMargin,
        );
        effectiveRequest = request.copyWith(context: contextBudget.context);
        AndroidFfiRuntimeProvider._log(
          '[TOKEN_BUDGET_DART] session=$sessionId'
          ' original_turns=${request.context.length}'
          ' selected_turns=${contextBudget.context.length}'
          ' trimmed_turns=${contextBudget.trimmedTurns}'
          ' prompt_tokens=${contextBudget.promptTokens}'
          ' prompt_budget_tokens=${contextBudget.promptBudgetTokens}'
          ' requested_generation_tokens=$boundedMaxTokens'
          ' available_generation_tokens=${contextBudget.availableGenerationTokens}'
          ' n_ctx=${LlamaNativeDefaults.nCtx}'
          ' safety_margin=${LlamaNativeDefaults.promptTokenSafetyMargin}'
          ' fits_requested_generation=${contextBudget.fitsRequestedGeneration}',
        );
      } catch (error) {
        _classifyFirstTokenTermination(
          flowState: flowState,
          reason: 'exact_token_context_selection_failed',
          boundary: 'tokenizer_readiness',
          exception: true,
        );
        AndroidFfiRuntimeProvider._log(
          '[TOKEN_BUDGET_DART_FAIL] session=$sessionId error=$error',
        );
        _updateRuntimeStatus(
          LocalRuntimeStatus.failed,
          message: 'Exact tokenizer context selection failed.',
        );
        AndroidFfiRuntimeProvider._finishWithRuntimeError(
          controller,
          stage: 'tokenizer_readiness',
          message: 'Exact tokenizer context selection failed before inference.',
          details: error.toString(),
        );
        return null;
      }
    ...
    int exactPromptTokens;
    try {
      exactPromptTokens = bindings.countTokens(nativeSessionId, prompt);
    } catch (error) {
      _classifyFirstTokenTermination(
        flowState: flowState,
        reason: 'exact_prompt_token_count_failed',
        boundary: 'tokenizer_readiness',
        exception: true,
      );
      AndroidFfiRuntimeProvider._log(
        '[TOKEN_COUNT_EXACT_FAIL] session=$sessionId error=$error',
      );
      _updateRuntimeStatus(
        LocalRuntimeStatus.failed,
        message: 'Exact tokenizer readiness check failed.',
      );
      AndroidFfiRuntimeProvider._finishWithRuntimeError(
        controller,
        stage: 'tokenizer_readiness',
        message: 'Exact tokenizer readiness check failed before inference.',
        details: error.toString(),
      );
      return null;
    }
    ...
    if (exactPromptTokens <= 0) {
      ...
      AndroidFfiRuntimeProvider._log(
        '[FFI_BRANCH] session=$sessionId name=tokenizer_readiness_failed',
      );
      _updateRuntimeStatus(
        LocalRuntimeStatus.failed,
    ...
    final availableGenerationTokens = LlamaNativeDefaults.nCtx -
        exactPromptTokens -
        LlamaNativeDefaults.promptTokenSafetyMargin;
    if (availableGenerationTokens <= 0) {
      _classifyFirstTokenTermination(
        flowState: flowState,
        reason: 'prompt_exceeds_native_context',
        boundary: 'tokenizer_readiness',
      );
      AndroidFfiRuntimeProvider._log(
        '[TOKEN_BUDGET_DART_FAIL] session=$sessionId'
        ' reason=prompt_exceeds_native_context'
      ...
      _updateRuntimeStatus(
        LocalRuntimeStatus.failed,
        message: 'Prompt is too large for the selected local model context.',
      );
      AndroidFfiRuntimeProvider._finishWithRuntimeError(
        controller,
        stage: 'tokenizer_readiness',
    ...
    final maxTokens = availableGenerationTokens < boundedMaxTokens
    ...
    AndroidFfiRuntimeProvider._log(
      '[TOKENIZER] status=begin prompt_chars=${prompt.length}',
    );
    AndroidFfiRuntimeProvider._log(
      '[TOKEN_COUNT] exact_prompt_tokens=$exactPromptTokens'
      ' prompt_word_estimate=$promptWordEstimate prompt_chars=${prompt.length}',
    );
    AndroidFfiRuntimeProvider._log(
      '[TOKENIZER_OK] exact_prompt_tokens=$exactPromptTokens',
    );
    AndroidFfiRuntimeProvider._log(
      '[MODEL_EXECUTION] tokenization start prompt_chars=${prompt.length}'
      ' exact_prompt_tokens=$exactPromptTokens',
    );
    AndroidFfiRuntimeProvider._log(
      '[CONTEXT_SIZE] session=$sessionId'
      ' context_lines=${effectiveRequest.context.length}'
      ' original_context_lines=${request.context.length}'
      ' system_chars=${(request.systemPrompt ?? '').length}'
      ' prompt_chars=${request.prompt.length}'
      ' composed_prompt_chars=${prompt.length}'
      ' exact_prompt_tokens=$exactPromptTokens',
    );
    AndroidFfiRuntimeProvider._log(
      '[KV_CACHE] layer=native status=managed_by_llama_bridge',
    );
    AndroidFfiRuntimeProvider._log(
      '[PROMPT_EVAL] stage=start prompt_chars=${prompt.length}'
      ' exact_prompt_tokens=$exactPromptTokens',
    );
    AndroidFfiRuntimeProvider._log(
      '[TOKEN_BUDGET_DART_FINAL] session=$sessionId'
      ' prompt_tokens=$exactPromptTokens'
      ' requested_generation_tokens=$boundedMaxTokens'
      ' effective_generation_tokens=$maxTokens'
      ' available_generation_tokens=$availableGenerationTokens'
      ' n_ctx=${LlamaNativeDefaults.nCtx}'
      ' safety_margin=${LlamaNativeDefaults.promptTokenSafetyMargin}'
      ' generation_clamped=${maxTokens != boundedMaxTokens}',
    );
    ...
    final firstTokenDeadline = isForensicSelfTest
        ...
        : AndroidFfiRuntimeProvider._firstTokenTimeout;
    if (request.maxTokens > AndroidFfiRuntimeProvider._safeMaxTokens) {
      AndroidFfiRuntimeProvider._log(
        '[MODEL_EXECUTION] requested max_tokens=${request.maxTokens} exceeds safe limit, clamped to $boundedMaxTokens',
      );
    }
    if (maxTokens < boundedMaxTokens) {
      AndroidFfiRuntimeProvider._log(
        '[MODEL_EXECUTION] generation headroom reduced by exact token budget'
        ' requested=$boundedMaxTokens effective=$maxTokens',
      );
    }

    final loadedCheck = bindings.sessionIsActive(nativeSessionId);
    AndroidFfiRuntimeProvider._log(
      '[MODEL_EXECUTION] llb_session_is_active before start_generation: $loadedCheck',
    );
    if (loadedCheck != 1) {
      _classifyFirstTokenTermination(
        flowState: flowState,
      ...
      AndroidFfiRuntimeProvider._log(
        '[FFI_BRANCH] session=$sessionId name=session_inactive_before_start',
      );
      final nativeErr =
          AndroidFfiRuntimeProvider._safeLastError(bindings, nativeSessionId);
      _updateRuntimeStatus(
    ...
    try {
      ...
      final nativeHandleAddress = nativeSessionId > 0
      ...
      AndroidFfiRuntimeProvider._log(
        '[FFI_PRE_START] session=$sessionId native_session=$nativeSessionId',
      );
      AndroidFfiRuntimeProvider._log(
        '[FORENSIC_BEFORE_START_GENERATION] sessionId=$sessionId nativeSessionId=$nativeSessionId',
      );
      AndroidFfiRuntimeProvider._log(
      ...
      AndroidFfiRuntimeProvider._log(
        '[FORENSIC_AFTER_START_GENERATION] sessionId=$sessionId nativeSessionId=$nativeSessionId startResult=$startResult',
      );
      ...
      AndroidFfiRuntimeProvider._log(
        '[FFI_EXCEPTION] session=$sessionId stage=start_generation error=$error',
      );
      _setPhase(RuntimePhase.failed);
      await _safeResetRuntime(
        bindings,
        reason: 'start_generation_exception',
      );
      _updateRuntimeStatus(
    ...
    startupWatch.stop();
    AndroidFfiRuntimeProvider._log(
      '[MODEL_EXECUTION] llb_session_start_gen returned: $startResult',
    );
    ...
    if (startResult != 0) {
      ...
      AndroidFfiRuntimeProvider._log(
        '[FFI_BRANCH] session=$sessionId name=start_generation_failed_code',
      );
      freePromptNativePtr();
      _setPhase(RuntimePhase.failed);
      final err =
      ...
      await _safeResetRuntime(
        bindings,
        reason: 'start_generation_failed',
      );
    ...
    AndroidFfiRuntimeProvider._log(
      '[WARMUP] inference_startup_ok session=$sessionId'
    ...
    return _GenerationStartupState(
      controller: controller,
