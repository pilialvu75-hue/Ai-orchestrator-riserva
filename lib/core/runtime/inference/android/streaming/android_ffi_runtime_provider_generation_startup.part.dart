...
extension AndroidFfiRuntimeGenerationStartupExtension on AndroidFfiRuntimeProvider {
  Future<_GenerationStartupState?> _prepareGenerationStartup({
    ...
    _log('[MODEL_EXISTS] path=$resolvedModelPath exists=$modelExists');
    if (modelExists) {
      ...
      _log('[MODEL_SIZE] path=$resolvedModelPath size_bytes=$modelSizeBytes');
      _log('[MODEL_READABLE] path=$resolvedModelPath readable=$modelReadable');
    } else {
      _log('[MODEL_SIZE] path=$resolvedModelPath size_bytes=N/A (file not found)');
      _log('[MODEL_READABLE] path=$resolvedModelPath readable=false (file not found)');
    }
    ...
    if (modelValidationError != null) {
      ...
      _log('[GGUF] validation=failed path=$resolvedModelPath reason=$modelValidationError');
      _log('[TERMINAL_STATE] state=failed reason=model_validation' ' path=$resolvedModelPath error=$modelValidationError');
      clearRuntimeVerification();
    ...
    if (!isForensicSelfTest) {
      ...
      _log( '[AI_RUNTIME_MONITOR] FORENSIC - File: android_ffi_runtime_provider.dart | Line: 689 | Function: streamInference() | BEFORE calling _ensureWarmup()', );
      final warmupReady = await _ensureWarmup( sessionId: sessionId, modelPath: resolvedModelPath, );
      _log('[FORENSIC_AFTER_WARMUP]');
      _log( '[AI_RUNTIME_MONITOR] FORENSIC - File: android_ffi_runtime_provider.dart | Line: 696 | Function: streamInference() | AFTER calling _ensureWarmup()', );
      if (!warmupReady) {
    ...
    _log('[NATIVE_MODEL_LOAD_BEGIN] path=$resolvedModelPath modelId=$modelId' ' n_ctx=${LlamaNativeDefaults.nCtx} n_threads=${LlamaNativeDefaults.nThreads}' ' gpu_layers=${LlamaNativeDefaults.nGpuLayers}');
    int nativeSessionId;
    try {
      _setPhase(RuntimePhase.tokenizing);
      _log('[FIRST_FFI_CALL_BEGIN] stage=session_create phase=$_currentFfiPhase');
      _log('[FFI_PRE_CREATE_SESSION] session=$sessionId path=$resolvedModelPath');
      _log( '[FORENSIC_BEFORE_CREATE_SESSION] sessionId=$sessionId modelId=$modelId modelPath=$resolvedModelPath', );
      _log( '[FIRST_TOKEN_SESSION_CREATE_BEGIN] attemptId=${_currentFirstTokenAttemptId ?? 'unknown'}' ' sessionId=$sessionId modelId=$modelId', );
      flowState.firstFfiInvocationAttempted = true;
      _log('[FFI_CREATE_SESSION] path=$resolvedModelPath');
      ...
      _log( '[FORENSIC_AFTER_CREATE_SESSION] nativeSessionId=$nativeSessionId', );
      _log( '[FIRST_TOKEN_SESSION_CREATE_END] attemptId=${_currentFirstTokenAttemptId ?? 'unknown'}' ' sessionId=$sessionId nativeSessionId=$nativeSessionId', );
      ...
      _log('[SESSION_CREATE_FAIL] path=$resolvedModelPath exception=$error');
      ...
      _finishWithRuntimeError( controller, stage: 'session_create', message: 'Session create failed.', details: error.toString(), );
      return null;
    }
    _log('[SESSION_CREATE_OK] session=$nativeSessionId path=$resolvedModelPath');
    _log('[FFI_CREATE_SESSION_OK] session=$nativeSessionId path=$resolvedModelPath');
    ...
    if (nativeSessionId <= 0 || activeAfterCreate != 1) {
      ...
      _log('[SESSION_CREATE_FAIL] code=$nativeSessionId error=$errMsg path=$resolvedModelPath');
      ...
      _finishWithRuntimeError( controller, stage: 'session_create', message: 'Failed to create runtime session.', details: 'Create failed with code $nativeSessionId: $errMsg', );
      return null;
    }
    _log('[NATIVE_MODEL_LOAD_SUCCESS] path=$resolvedModelPath modelId=$modelId' ' session=$nativeSessionId');
    _log('[NATIVE_CONTEXT_CREATE] path=$resolvedModelPath status=ok');
    ...
    if (promptWordEstimate <= 0) {
      ...
      _finishWithRuntimeError( controller, stage: 'tokenizer_readiness', message: 'Tokenizer readiness check failed before inference.', );
      return null;
    }
    ...
    _log( '[TOKEN_COUNT] prompt_word_estimate=$promptWordEstimate prompt_chars=${prompt.length}', );
    _log('[TOKENIZER_OK] prompt_word_estimate=$promptWordEstimate');
    _log( '[MODEL_EXECUTION] tokenization start prompt_chars=${prompt.length} prompt_word_estimate=$promptWordEstimate', );
    _log( '[CONTEXT_SIZE] session=$sessionId context_lines=${request.context.length} system_chars=${(request.systemPrompt ?? '').length} prompt_chars=${request.prompt.length} composed_prompt_chars=${prompt.length}', );
    _log('[KV_CACHE] layer=native status=managed_by_llama_bridge');
    _log( '[PROMPT_EVAL] stage=start prompt_chars=${prompt.length} prompt_word_estimate=$promptWordEstimate', );
    ...
    if (request.maxTokens > AndroidFfiRuntimeProvider._safeMaxTokens) {
      _log( '[MODEL_EXECUTION] requested max_tokens=${request.maxTokens} exceeds safe limit;
    ...
    if (loadedCheck != 1) {
      ...
      _finishWithRuntimeError( controller, stage: 'start_generation', message: 'Session is not active in the native runtime (llb_session_is_active=$loadedCheck).', details: nativeErr.isNotEmpty ? nativeErr : null, );
      return null;
    }
    _log(
    ...
    try {
      ...
      _finishWithRuntimeError( controller, stage: 'start_generation', message: error is TimeoutException ? 'Native generation start timed out.' : 'Native generation start failed.', details: error.toString(), );
      return null;
    }
    startupWatch.stop();
    ...
    if (startupWatch.elapsed > AndroidFfiRuntimeProvider._startGenerationTimeout) {
      ...
      _log( '[FFI_TIMEOUT] session=$sessionId stage=start_generation_postcheck' ' timeout_ms=${AndroidFfiRuntimeProvider._startGenerationTimeout.inMilliseconds}', );
      freePromptNativePtr();
      ...
      _finishWithRuntimeError( controller, stage: 'start_generation', message: 'Inference startup timed out after ${AndroidFfiRuntimeProvider._startGenerationTimeout.inSeconds}s.', );
      return null;
    }
    if (startResult != 0) {
      ...
      _finishWithRuntimeError( controller, stage: 'start_generation', message: 'Failed to start generation.', details: err, );
      return null;
    }
    _log('[WARMUP] inference_startup_ok session=$sessionId' ' startup_ms=${startupWatch.elapsed.inMilliseconds}');
    _log( '[PROMPT_EVAL] stage=ready startup_ms=${startupWatch.elapsed.inMilliseconds}', );
