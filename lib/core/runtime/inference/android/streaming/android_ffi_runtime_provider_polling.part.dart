    ...
    try {
      ...
      while (true) {
        ...
        try {
          ...
          AndroidFfiRuntimeProvider._finishWithRuntimeError( controller, stage: 'poll_token', message: 'Native poll_token failed.', details: error.toString(), ); break; }
        ...
        if (status == 1) {
          ...
          try {
            ...
            if (state.consecutiveInvalidTokens >= AndroidFfiRuntimeProvider._maxConsecutiveInvalidTokens) {
              ...
              AndroidFfiRuntimeProvider._finishWithRuntimeError( controller, stage: 'token_decode', message: 'Invalid generated token stream.', details: error.toString(), ); break; } continue; }
          final trimmedPiece = piece.trim();
          final tokenObservedAt = DateTime.now();
          state.lastNativeActivityAt = tokenObservedAt;
          if (_shouldIgnoreToken(trimmedPiece)) { continue; }
          final sanitizedPiece = _sanitizeStructuralTemplateOutput(piece);
          final trimmedSanitizedPiece = sanitizedPiece.trim();
          if (trimmedSanitizedPiece.isEmpty) { continue; }
          if (_isDeveloperMode) {
          ...
          AndroidFfiRuntimeProvider._log( '[DART_TOKEN_RECEIVED] elapsed_ms=${streamingElapsed.inMilliseconds} thread_id=$dartThreadId token_id=-1 token_text_length=${sanitizedPiece.length} queue_size=-1 poll_iteration=${attemptState.pollIterations}', );
          ...
          AndroidFfiRuntimeProvider._log( '[TOKEN_STREAM] piece token_index=${attemptState.estimatedTokens} text="${sanitizedPiece.replaceAll('\n', r'\n')}"' ' total_chars=${state.fullText.length} since_first_token_ms=${sinceFirstToken?.inMilliseconds ?? 0}', );
          ...
          AndroidFfiRuntimeProvider._log( '[TOKEN_DECODE] token_index=${attemptState.estimatedTokens} chars=${sanitizedPiece.length}' ' text="${sanitizedPiece.replaceAll('\n', r'\n')}"', );
          if (sanitizedPiece == state.lastPiece) {
            ...
            if (state.repeatedTokenCount >= AndroidFfiRuntimeProvider._maxRepeatedTokenLoop) {
              ...
              _setPhase(RuntimePhase.failed);
              AndroidFfiRuntimeProvider._log( '[STREAM_LOOP] reason=repeated_token' ' count=${state.repeatedTokenCount} token="${sanitizedPiece.replaceAll('\n', r'\n')}"' ' generated_tokens=${attemptState.estimatedTokens} session=$sessionId', );
              AndroidFfiRuntimeProvider._log( '[TERMINAL_STATE] state=failed reason=repeated_token_loop' ' generated_tokens=${attemptState.estimatedTokens}' ' elapsed_ms=${streamingElapsed.inMilliseconds}', );
              ...
              AndroidFfiRuntimeProvider._finishWithRuntimeError( controller, stage: 'generation_loop', message: 'Repeated-token loop detected.', details: 'token="$sanitizedPiece"', ); break; }
          } else {
          ...
          AndroidFfiRuntimeProvider._log( '[TOKEN_EMIT] token_index=${attemptState.estimatedTokens} chars=${sanitizedPiece.length}' ' session=$sessionId', );
          AndroidFfiRuntimeProvider._log( '[DART_STREAM_RENDER] elapsed_ms=${streamingElapsed.inMilliseconds} thread_id=$dartThreadId token_id=-1 token_text_length=${sanitizedPiece.length} queue_size=-1 poll_iteration=${attemptState.pollIterations} subscription_alive=${!controller.isClosed}', );
          ...
          AndroidFfiRuntimeProvider._log( '[TERMINAL_STATE] state=success generated_tokens=${attemptState.estimatedTokens}' ' elapsed_ms=${completedElapsed.inMilliseconds}', );
          ...
          _updateRuntimeStatus( LocalRuntimeStatus.completed, message: 'Completed', tokensGenerated: attemptState.estimatedTokens, elapsed: completedElapsed, startedAt: state.startedAt, ); break;
        } else if (status == -99) {
          ...
          _updateRuntimeStatus( LocalRuntimeStatus.runtimeUnavailable, tokensGenerated: attemptState.estimatedTokens, elapsed: DateTime.now().difference(state.startedAt), ); break;
        } else if (status == -1) {
          ...
          AndroidFfiRuntimeProvider._log( '[TERMINAL_STATE] state=native_error generated_tokens=${attemptState.estimatedTokens}' ' elapsed_ms=${DateTime.now().difference(state.startedAt).inMilliseconds}' ' error=$err', );
          ...
          if ((statusLower.contains('timeout') || statusLower.contains('stalled')) &&
            ...
            if (!controller.isClosed) {
              AndroidFfiRuntimeProvider._finishWithRuntimeError( controller, stage: 'generation', message: err.isNotEmpty ? err : 'Inference failed.', ); } } break;
        } else {
