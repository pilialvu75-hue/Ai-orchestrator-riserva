    ...
    try {
      ...
      while (true) {
        ...
        if (cancellationToken.isCancelled || controller.isClosed) {
          ...
          _log('[FFI_RUNTIME_UNAVAILABLE_REASON] session=$sessionId reason=pre_poll_cancellation'); break; }
        ...
        try {
          ...
          _finishWithRuntimeError( controller, stage: 'poll_token', message: 'Native poll_token failed.', details: error.toString(), ); break; }
        ...
        if (status == 1) {
          ...
          try {
            ...
            if (state.consecutiveInvalidTokens >= AndroidFfiRuntimeProvider._maxConsecutiveInvalidTokens) {
              ...
              _finishWithRuntimeError( controller, stage: 'token_decode', message: 'Invalid generated token stream.', details: error.toString(), ); break; } continue; }
          ...
          if (sanitizedPiece == state.lastPiece) {
            ...
            if (state.repeatedTokenCount >= AndroidFfiRuntimeProvider._maxRepeatedTokenLoop) {
              ...
              _finishWithRuntimeError( controller, stage: 'generation_loop', message: 'Repeated-token loop detected.', details: 'token="$sanitizedPiece"', ); break; }
          } else {
          ...
          if ((statusLower.contains('timeout') || statusLower.contains('stalled')) &&
            ...
            if (!controller.isClosed) {
              _finishWithRuntimeError( controller, stage: 'generation', message: err.isNotEmpty ? err : 'Inference failed.', ); } } break;
        } else {
