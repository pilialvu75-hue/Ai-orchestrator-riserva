...
String? publicLogProjection(String line) {
  ...
  if (event == 'WORKSHOP_ENGINEER_PROMPT') {
    final m = RegExp(
      ...
      r'workspace_files=(\d{1,6}) architect_chars=(\d{1,9})  // text and arbitrary stream payloads must never leave the device.
...
,
  ...
  if (event == 'WORKSHOP_ENGINEER_RETRY') {
    final m = RegExp(
      ...
      r'attempt=(\d{1,3}) '
      r'terminal=(success|timeout|failed|cancelled|modelUnavailable|none)  // text and arbitrary stream payloads must never leave the device.
  if (event == 'TOKEN_STREAM') {
    final providerNotice = RegExp(
      r'^notice session=[A-Za-z0-9._:-]{1,80} '
      r'notice="cloud_provider:(openAi|gemini|claude|grok|copilot|groq|nvidiaNim|mistral|openRouter)"$',
