...
/// Remote data source for OpenRouter's OpenAI-compatible API.
///
/// `openrouter/free` is intentionally the conservative default: OpenRouter
/// routes only among models currently exposed at zero token price. The
/// response's concrete model id is preserved by [AiResponseModel], allowing a
/// higher layer to deduplicate model families when collecting independent
/// "eyes" from several providers.
class OpenRouterDataSource {
  OpenRouterDataSource({
    ...
    this.model = 'openrouter/free',
            ...
            (() => CloudCredentialStore.instance.secretFor('openRouter') ?? apiKey),
  ...
  Future<AiResponseModel> complete(AiRequestModel request) async {
    ...
    if (credential.isEmpty) {
      throw const ServerException('OpenRouter API key not configured');
    }
