...
/// Remote data source for Mistral Studio's Chat Completions API.
///
/// Mistral Studio currently offers a Free mode with limited usage, but model
/// availability is account/tier dependent. The conservative default mirrors
/// Mistral's Free-mode quickstart (`mistral-small-latest`). Stronger models may
/// be selected explicitly after the account confirms they are available.
class MistralDataSource {
  MistralDataSource({
    ...
    this.model = 'mistral-small-latest',
            ...
            (() => CloudCredentialStore.instance.secretFor('mistral') ?? apiKey),
  ...
  Future<AiResponseModel> complete(AiRequestModel request) async {
    ...
    if (credential.isEmpty) {
      throw const ServerException('Mistral API key not configured');
    }
