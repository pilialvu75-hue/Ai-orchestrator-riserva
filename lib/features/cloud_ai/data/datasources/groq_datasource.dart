...
/// Remote data source for GroqCloud's OpenAI-compatible Chat Completions API.
///
/// Groq is intentionally a distinct provider from xAI/Grok. The default model
/// is a coding/reasoning-oriented model currently available on Groq's Free
/// Plan. Provider cost/quota policy is not decided here; routing owns that
/// responsibility.
class GroqDataSource {
  GroqDataSource({
    ...
    this.model = 'qwen/qwen3.8-27b',
            ...
            (() => CloudCredentialStore.instance.secretFor('groq') ?? apiKey),
  ...
  Future<AiResponseModel> complete(AiRequestModel request) async {
    ...
    if (credential.isEmpty) {
      throw const ServerException('Groq API key not configured');
    }
