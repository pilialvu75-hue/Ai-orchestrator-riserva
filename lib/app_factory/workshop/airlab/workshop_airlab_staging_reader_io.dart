...
/// Native read-only staging reader. It never receives the real repository root.
final class WorkshopAirLabIoStagingReader
    implements WorkshopAirLabStagingReader {
  const WorkshopAirLabIoStagingReader({this.maxFileBytes = 256 * 1024});
  ...
  Future<WorkshopAirLabStagedFile> inspect({
    ...
    if (rootType == FileSystemEntityType.link) {
      throw WorkshopAirLabPromotionException(
        ...
        code: 'staging_root_symlink', path: rootValue);
    }
    if (rootType != FileSystemEntityType.directory) {
      throw WorkshopAirLabPromotionException(
        'Cantiere staging root must be an existing directory.',
        code: 'staging_root_invalid', path: rootValue);
    }
    try {
      final canonicalRoot =
          p.normalize(await Directory(rootPath).resolveSymbolicLinks());
      ...
      if (!_isInside(rootPath, absoluteTarget)) {
        throw WorkshopAirLabPromotionException(
          ...
          code: 'path_escape', path: normalizedPath);
      }
      ...
      if (targetType == FileSystemEntityType.link) {
        throw WorkshopAirLabPromotionException(
          ...
          code: 'symlink_target_forbidden', path: normalizedPath);
      }
      if (targetType != FileSystemEntityType.file) {
        throw WorkshopAirLabPromotionException(
          'AIrLab staged changed path must resolve to a file or be absent.',
          code: 'staged_path_type_invalid', path: normalizedPath);
      }
      ...
      if (!_isInside(canonicalRoot, resolvedTarget)) {
        throw WorkshopAirLabPromotionException(
          ...
          code: 'symlink_escape', path: normalizedPath);
      }
      final length = await file.length();
      if (length > maxFileBytes) {
        throw WorkshopAirLabPromotionException(
          'AIrLab staged file exceeds the promotion read limit.',
          code: 'staged_file_too_large', path: normalizedPath);
      }
      ...
      try {
        return WorkshopAirLabStagedFile.present(
          utf8.decode(bytes, allowMalformed: false));
      } on FormatException {
        throw WorkshopAirLabPromotionException(
          'AIrLab staged file is not valid UTF-8 text.',
          code: 'staged_file_not_utf8', path: normalizedPath);
      }
    ...
    for (var index = 0; index < segments.length; index += 1) {
      ...
      if (type == FileSystemEntityType.link) {
        throw WorkshopAirLabPromotionException(
          ...
          code: 'symlink_escape', path: relativePath);
      }
      ...
      if (index < segments.length - 1 &&
        ...
        throw WorkshopAirLabPromotionException(
          ...
          code: 'staged_path_type_invalid', path: relativePath);
      }
    }
  }
...
String _normalizeRelativePath(String rawPath) {
  ...
  if (value.isEmpty) {
    throw const WorkshopAirLabPromotionException(
      'AIrLab staging read path is empty.', code: 'empty_path');
  }
  if (value.contains('\u0000')) {
    throw WorkshopAirLabPromotionException(
