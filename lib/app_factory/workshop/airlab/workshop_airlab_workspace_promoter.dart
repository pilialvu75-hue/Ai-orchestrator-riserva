...
/// Snapshot of one path inside the isolated AIrLab staging area.
///
/// The staging reader owns all platform-specific filesystem/storage access.
/// This contract intentionally contains no `dart:io` dependency so Web can
/// provide a browser storage implementation later without changing promotion.
final class WorkshopAirLabStagedFile {
  const WorkshopAirLabStagedFile._({
    required this.exists,
    this.content,
  });

  const WorkshopAirLabStagedFile.missing()
      : this._(exists: false);
...
/// Read-only view of the controlled AIrLab staging area.
...
/// Fail-closed promotion failure.
final class WorkshopAirLabPromotionException implements Exception {
  const WorkshopAirLabPromotionException(
    this.message, {
    required this.code,
    this.path,
  });
...
/// Result of staging -> VirtualWorkspace promotion.
///
/// Promotion is still non-mutating with respect to the real repository.
...
/// Promotes an already validated A2 staging transaction into VirtualWorkspace.
///
/// This boundary deliberately stops at `WorkspaceSession.review`:
///
///   AIrLab staging
///       ↓
...
///   VirtualWorkspace
///       ↓
///   review / validation / explicit approval / apply
///
/// It never calls `approveApply()` or `apply()` and therefore cannot mutate the
/// backing repository by itself.
final class WorkshopAirLabWorkspacePromoter {
  ...
  Future<WorkshopAirLabWorkspacePromotionResult> promote({
    ...
    final declaredOperationCount =
        executionResult.metadata['operationCount'];
    ...
    // Inspect every staged path before the first VirtualWorkspace mutation.
...
String _normalizeRelativePath(String rawPath) {
  ...
  if (value.isEmpty) {
    throw const WorkshopAirLabPromotionException(
      'AIrLab promotion path is empty.',
      code: 'empty_path',
    );
  }
  if (value.contains('\u0000')) {
