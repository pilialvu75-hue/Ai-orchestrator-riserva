...
///
/// Snapshot capture excludes build outputs, dependency caches, VCS metadata and
/// common credential/signing files. Reuse never writes directly to the live
/// project: [stageInto] writes only to VirtualWorkspace and remains subject to
/// the existing review/validation/owner-approval/apply gates.
final class WorkshopReuseSourceSnapshotService {
  ...
  static const Set<String> _excludedDirectories = <String>{
    '.git', '.dart_tool', '.gradle', '.idea', '.vscode', '.cache',
    'build', 'node_modules', 'coverage', 'dist', 'out', 'pods', 'deriveddata',
  };

  static const Set<String> _blockedFileNames = <String>{
    '.env', 'key.properties', 'local.properties', 'gradle.properties',
  ...
  static const Set<String> _allowedExtensions = <String>{
    '.dart', '.yaml', '.yml', '.json', '.md', '.txt', '.html', '.htm',
    '.css', '.scss', '.sass', '.js', '.mjs', '.cjs', '.ts', '.tsx', '.jsx',
    '.xml', '.gradle', '.kts', '.java', '.kt', '.swift', '.m', '.mm', '.h',
    '.hpp', '.c', '.cc', '.cpp', '.cmake', '.toml', '.ini', '.conf',
    '.properties', '.lock', '.sh', '.bat', '.ps1', '.sql', '.graphql', '.gql',
  };

  static const Set<String> _allowedExtensionlessNames = <String>{
    'gradlew', 'makefile', 'dockerfile', 'license', 'readme',
  };
  ...
  Future<WorkshopReuseSourceSnapshot> capture({
    ...
    final finalDirectory = Directory(
      p.join(snapshotsRoot.path, _safeAssetDirectoryName(normalizedAssetId)),
    );
  ...
  static bool _isSafeReusablePath(String relativePath) {
    ...
    for (final segment in segments.take(segments.length - 1)) {
      if (_excludedDirectories.contains(segment.toLowerCase())) return false;
    }
  ...
  static String _safeAssetDirectoryName(String assetId) {
    final normalized = assetId
        ...
        .replaceAll(RegExp(r'^[-.]+|[-.]+$'), '');
    if (normalized.isEmpty) return 'reuse-${assetId.hashCode.abs()}';
