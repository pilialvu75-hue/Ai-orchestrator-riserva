...
/// Research lanes used by Cantiere before implementation planning.
///
/// Each lane answers a different question:
/// - what comparable products already do well;
/// - what users praise or complain about;
/// - what the application domain requires or commonly contains.
...
final class WorkshopWebEvidencePack {
  ...
  /// Converts verified tool output into bounded, explicitly untrusted context
  /// for the reasoning roles. URLs remain in the WebSearchTool output so the
  /// Architect can preserve provenance in later decisions.
...
/// Best-effort, read-only Web research for Cantiere preflight.
///
/// The service never mutates a workspace or the Module Library. It gathers a
/// small evidence pack that can improve Orchestrator/Architect reasoning while
/// preserving offline-first execution: strict offline mode skips research and
/// individual provider failures never block the preflight.
final class WorkshopWebResearchService {
  ...
  bool shouldResearch(WorkshopRequest request) {
    ...
    // exists. Narrow file/component creation stays local unless research is
