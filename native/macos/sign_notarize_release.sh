...
echo "Signing nested macOS code with: $identity"
...
# Sign loose Mach-O libraries/helpers first, deepest nested bundles next, and
# the outer application last. The app receives the release entitlements.
...
# Signing, notarization and stapling mutate the container. The public digest is
# deliberately computed only after every mutation and Gatekeeper assessment.
